...
const AnswerComponent: FC<AnswerComponentProps> = ({answer, index, letter, backgroundLetterColor, onResponseAnswer, changeOpacity}) => {
  return (
    <div className="flex m-[10px] w-[800px] min-w-[500px]">
      <div className="flex flex-[1] hover:scale-105 cursor-pointer transition-all" onClick={() => onResponseAnswer(index)}>
        <div style={{backgroundColor: answer.selected ? answer.correct ? '#2BAA6D' : '#E44A4C' : 'white', opacity: changeOpacity ? 0.7 : 1 }} className="flex h-full w-full gap-4 items-center p-3 rounded-md text-black transition-colors">
